...
STATE_FILE = "fbox_state.json"  # GitHub Actions usa directorio temporal
TIME_FILE = "last_report_time.json"  # Archivo para rastrear último reporte completo
HISTORY_FILE = "fbox_history.json"  # Archivo para datos históricos semanales
WEEKLY_TIME_FILE = "last_weekly_report.json"  # Archivo para rastrear último reporte semanal
...
def should_send_weekly_report():
    """Determina si debe enviarse el reporte semanal (cada lunes)"""
    now = now_paraguay()
    ...
    try:
        last_dt = datetime.fromisoformat(last_time)
        # Verificar si han pasado al menos 6 días desde el último reporte
...
if __name__ == "__main__":
    ...
    # Enviar reporte semanal (cada lunes)
    if should_send_weekly_report():
        ...
        print(f"⏭️ Reporte semanal omitido (se envía cada {['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'][WEEKLY_REPORT_DAY]})")
